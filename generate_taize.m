...
% SET YOUR INPUT HERE !!!
% ============================     % ============================
target_duration = 600; % seconds   % <<<----- set your desired duration
% ============================     %   the output song will be just
% SET YOUR INPUT HERE !!!          %   slightly longer than it
% ============================     % ============================


[y1,Fs] = audioread('src.mp3'); % base source song
[y2,~] = audioread('rep.mp3'); % replicate of the song (or part of the
                               % song with the opening / intro suppressed

t_delay = 40.548; % seconds
d = ceil(Fs * t_delay); % digital shift

SONG = y1;

replicate_cnt = 0;
while (length(SONG) / Fs < target_duration)
    SONG = [SONG ; zeros(d,1)] ; %#ok<AGROW>
    SONG = SONG + [zeros(d * (replicate_cnt+1),1) ; y2 ];
    replicate_cnt = replicate_cnt + 1;
end

fprintf('final duration: %f min\n', length(SONG) / Fs / 60);
audiowrite('out.m4a',SONG,Fs);
% in linux or macos, convert to mp3 or other formats
% using the terminal command "ffmpeg -i out.m4a out.mp3"
